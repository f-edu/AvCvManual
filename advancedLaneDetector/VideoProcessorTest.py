...
# stream = cv.VideoCapture("test_videos/road.avi") #6 7 8
if stream.isOpened() == False:
    print ("Cannot open input video")
    exit()

#some image processing parameters
# cropArea = [180, 150, 450, 350] #[from left, from up, to right, to down ]
...
# line1LEnd = np.array([100, 180])
# #6R
# line1RStart = np.array([300, 80])
# line1REnd = np.array([300, 180])
#
# #VideoTest
# cropArea = [380, 350, 900, 700] #[from left, from up, to right, to down ]
# # cropArea = [0, 124, 637, 298]
# sensorsNumber = 100
# sensorsWidth = 70
# #6L
...
#RoadTest
cropArea = [0, 250, 640, 480] #[from left, from up, to right, to down ]
# cropArea = [0, 124, 637, 298]
sensorsNumber = 10
sensorsWidth = 30
#6L
line1LStart = np.array([200, 80])
line1LEnd = np.array([100, 200])
#6R
line1RStart = np.array([500, 80])
line1REnd = np.array([500, 250])
#7L
...
while(cv.waitKey(1) != 27):
    ...
    #read and crop
    flag, imgFull = stream.read()
    imgFull=cv.resize(imgFull, (int(640), int(480)))
    ...
    canny = cv.Canny(cv.cvtColor(np.uint8(img*255), cv.COLOR_RGB2GRAY), 60, 170)
